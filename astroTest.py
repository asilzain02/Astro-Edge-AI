...
#######################################################
# 🔹 YOLOv8 Vision Stub (Offline Simulation)
#######################################################
class VisionModule:
...
class CoreAI:
    def __init__(self, model_path):
        ...
        self.llm = Llama(model_path=model_path, n_ctx=2048, n_threads=6)  # FULLY OFFLINE
...
#######################################################
# 🔹 Whisper Voice Input (Offline)
#######################################################
class VoiceInput:
    def __init__(self):
        print("🎙 Loading Whisper model (tiny)…")
        self.model = whisper.load_model("tiny")  # OFFLINE speech-to-text
...
class AstroEdgeApp:
    def __init__(self, model_path):
        ...
        tk.Button(side_panel, text="🎙 Voice Input", command=self.record_voice_query,
                  bg="purple", fg="white", font=("Consolas", 10, "bold")).pack(pady=5, fill="x")
        tk.Button(side_panel, text="💾 Export Logs", command=self.save_all_logs,
    ...
    def record_voice_query(self):
        self._append_chat("🎙 Listening for voice query...\n", "magenta")
        threading.Thread(target=self._transcribe_voice, daemon=True).start()

    def _transcribe_voice(self):
        transcript = self.voice_input.listen_and_transcribe(duration=5)
        self._append_chat(f"🎙 Transcribed: {transcript}\n", "magenta")
        threading.Thread(target=self._get_ai_response, args=(transcript,), daemon=True).start()
